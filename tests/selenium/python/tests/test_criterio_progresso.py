...
def test_progresso_aumenta_ao_adicionar_criterio(sistema_logado):
    ...
    print(f"Progresso antes de adicionar critério: {progresso_antes}")

    menu.acessar_aba_planejamento()
    menu.acessar_aba_criterios()
    planning = PlanningPage(driver)
    campo_id = planning.campo_criterios_id()
    campo_descricao = planning.campo_criterios_descricao()
    campo_id.clear()
    campo_id.send_keys("C1")
    campo_descricao.clear()
    campo_descricao.send_keys("Critério de inclusão")
    planning.selecionar_tipo_criterio("Inclusão")
    planning.botao_salvar_criterios().click()
    time.sleep(2)
    ...
    print(f"Progresso após adicionar critério: {progresso_depois}")

    assert progresso_depois > progresso_antes, (
        f"O progresso deveria aumentar após adicionar critério, mas era {progresso_antes} e ficou {progresso_depois}."
    )
