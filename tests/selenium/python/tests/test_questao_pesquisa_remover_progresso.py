...
def test_progresso_reduz_ao_remover_questao_pesquisa(sistema_logado):
    ...
    print(f"Progresso antes de remover questão de pesquisa: {progresso_antes}")

    menu.acessar_aba_planejamento()
    menu.acessar_aba_questoes_pesquisa()
    planning = PlanningPage(driver)
    planning.remover_questao_pesquisa()
    time.sleep(2)
    ...
    print(f"Progresso após remover questão de pesquisa: {progresso_depois}")

    assert progresso_depois < progresso_antes, (
        f"O progresso deveria reduzir após remover questão de pesquisa, mas era {progresso_antes} e ficou {progresso_depois}."
    )
