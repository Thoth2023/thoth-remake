...
def test_progresso_aumenta_ao_adicionar_questao_pesquisa(sistema_logado):
    ...
    print(f"Progresso antes de adicionar questão de pesquisa: {progresso_antes}")
    ...
    campo_descricao.send_keys("Questão de pesquisa de teste")
    planning.botao_salvar_questao_pesquisa().click()
    time.sleep(2)
    ...
    print(f"Progresso após adicionar questão de pesquisa: {progresso_depois}")

    assert progresso_depois > progresso_antes, (
        f"O progresso deveria aumentar após adicionar questão de pesquisa, mas era {progresso_antes} e ficou {progresso_depois}."
    )
