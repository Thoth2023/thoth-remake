...
def test_progresso_aumenta_ao_adicionar_estrategia(sistema_logado):
    ...
    print(f"Progresso antes de adicionar estratégia: {progresso_antes}")

    menu.acessar_aba_planejamento()
    menu.acessar_aba_estrategia()
    planning = PlanningPage(driver)
    campo_estrategia = planning.campo_estrategia()
    driver.execute_script("arguments[0].innerText = 'Estratégia de busca automatizada';", campo_estrategia)
    ...
    print(f"Progresso após adicionar estratégia: {progresso_depois}")

    assert progresso_depois > progresso_antes, (
        f"O progresso deveria aumentar após adicionar estratégia, mas era {progresso_antes} e ficou {progresso_depois}."
    )
