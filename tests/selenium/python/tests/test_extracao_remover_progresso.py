...
def test_progresso_reduz_ao_remover_pergunta_extracao(sistema_logado):
    ...
    print(f"Progresso antes de remover pergunta de extração: {progresso_antes}")

    menu.acessar_aba_planejamento()
    menu.acessar_aba_extracao()
    planning = PlanningPage(driver)
    planning.remover_extracao()
    time.sleep(2)
    ...
    print(f"Progresso após remover pergunta de extração: {progresso_depois}")

    assert progresso_depois < progresso_antes, (
        f"O progresso deveria reduzir após remover pergunta de extração, mas era {progresso_antes} e ficou {progresso_depois}."
    )
