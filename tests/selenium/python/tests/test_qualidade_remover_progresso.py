...
def test_progresso_reduz_ao_remover_questao_qualidade(sistema_logado):
    ...
    print(f"Progresso antes de remover questão de qualidade: {progresso_antes}")

    menu.acessar_aba_planejamento()
    menu.acessar_aba_questao_qualidade()
    planning = PlanningPage(driver)
    planning.remover_questao_qualidade()
    time.sleep(2)
    ...
    print(f"Progresso após remover questão de qualidade: {progresso_depois}")

    assert progresso_depois < progresso_antes, (
        f"O progresso deveria reduzir após remover questão de qualidade, mas era {progresso_antes} e ficou {progresso_depois}."
    )
