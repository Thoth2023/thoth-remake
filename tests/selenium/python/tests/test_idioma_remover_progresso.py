...
def test_progresso_reduz_ao_remover_idioma(sistema_logado):
    ...
    print(f"Progresso antes de remover idioma: {progresso_antes}")

    menu.acessar_aba_planejamento()
    planning = PlanningPage(driver)
    planning.remover_idioma()
    time.sleep(2)
    ...
    print(f"Progresso após remover idioma: {progresso_depois}")

    assert progresso_depois < progresso_antes, (
        f"O progresso deveria reduzir após remover o idioma, mas era {progresso_antes} e ficou {progresso_depois}."
    )