    ...
    <div class="col-md-6">
        <div class="form-group">
            <form role="form" method="POST" action="{{ route('project.conducting-import-studies.imp.store', ['projectId' => $id_project]) }}" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <label for="database" class="form-control-label">{{ __('project/conducting-import-studies.form.database') }}</label>
                    <select class="form-control" name="database">
                        <option value="" disabled selected>{{ __('project/conducting-import-studies.form.select-database') }}</option>
                        @forelse ($databases as $database)
                            @if ($database->state == 'approved')
                                <option value="{{ $database->id_database }}">{{ $database->name }}</option>
                            @endif
                        @empty
                            <option disabled>{{ __('project/conducting-import-studies.form.no-databases') }}</option>
                        @endforelse
                    </select>
                </div>
