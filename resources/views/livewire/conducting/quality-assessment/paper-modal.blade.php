        ...
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <ul class='list-group'>
                        <li class='list-group-item d-flex'>
                            <div class='w-10 pl-2'>
                                <b>{{ __('project/conducting.quality-assessment.table.id' )}}</b>
                            </div>
                            <div class='w-50 pl-2 pr-2'>
                                <b>{{ __('project/conducting.quality-assessment.modal.table.description' )}}</b>
                            </div>
                            <div class='w-20 pl-2 ms-auto'>
                                <b>{!!__('project/conducting.quality-assessment.modal.table.min-to-app' )!!}</b>
                            </div>
                            <div class='w-20 pl-2 ms-auto'>
                                <b>{{ __('project/conducting.quality-assessment.modal.table.score' )}}</b>
                            </div>
                        </li>
                    </ul>
                    ...
                    <ul class='list-group list-group-flush'>
                        @foreach ($questions as $question)
                            <x-search.item wire:key="{{ $question->id_qa }}" target="search-papers"
                ...
                <span data-search>
                    <x-select
                        disabled="{{ !$canEdit }}"
                        wire:model.defer="selected_questions_score.{{ $question->id_qa }}"
                    >
                        {{-- Opção inicial se ainda não houver valor salvo --}}
                ...
                @if($canEdit)
                        <div class="mt-3 text-end">
                            <button type="button" class="btn btn-primary" wire:click="applyScores">
                                <i class="fa-solid fa-check"></i>
                                {{ __('project/conducting.quality-assessment.modal.apply-scores') }}
                            </button>
                        </div>
                    @endif
                    ...
                    <div class="d-flex flex-column mt-3">
                        ...
                        <textarea id="note" class="form-control" rows="2" wire:model="note" wire:blur="saveNote"
                                  ...
                                  @if(!$canEdit) disabled @endif required></textarea>
                    </div>
...
<script>
    $(document).ready(function () {
        ...
        // Reabrir o modal principal após fechar o modal de sucesso
        $('#successModalQuality').on('hidden.bs.modal', function () {
            $('#paperModalQuality').modal('show');
        });

        // Mostrar toast em vez de modal
